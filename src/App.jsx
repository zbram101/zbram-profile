      ...
      <div className="page-content">
        ...
        <section id="projects" className="section">
          ...
          <div className="project-grid">{projects.map((project,i) => <article className={`project-card project-${i}`} key={project.title}>
            <div className="project-visual" aria-hidden="true">{i === 0 ? <div className="match-art"><span>candidate</span><div className="match-line">↔</div><span>opportunity</span></div> : i === 1 ? <div className="radar-art"><i/><i/><span>SKIRMESH</span></div> : i === 2 ? <div className="fiji-art">FIJI<br/><span>FRY HOUSE</span><small>GOOD FOOD. GOOD COMPANY.</small></div> : <div className="bot-art"><MessageSquare size={38}/><span>Hello, curious human<span className="caret">_</span></span></div>}</div>
            <div className="project-body"><div className="project-category">{project.category}<span>{String(i+1).padStart(2,'0')}</span></div><h3>{project.title}</h3><p>{project.description}</p><details className="project-details"><summary>Behind the project <span>+</span></summary><ul>{project.accomplishments.map(item => <li key={item}>{item}</li>)}</ul></details>{i === 3 ? <a className="project-link" href="#contact">Try the assistant <ArrowUpRight size={17}/></a> : project.link ? <a className="project-link" href={project.link} target="_blank" rel="noreferrer">Visit project <ArrowUpRight size={17}/></a> : <span className="project-status">Exited · Recruiting platform</span>}</div>
          </article>)}</div>
        </section>
