...
export function Chat({onBusyChange}) {
  ...
  return <div className="chat-panel">
    ...
    <div ref={log} className="chat-messages" role="log" aria-label="Conversation" aria-live="polite" aria-relevant="additions text" aria-busy={busy}>
      {messages.map((m,i)=><div className={`chat-message ${m.role}`} key={i}><span className="message-label">{m.role==='user'?'YOU':'ZB / ASSISTANT'}</span><div className="message-content"><ReactMarkdown components={{a:({node,...props})=><a {...props} target="_blank" rel="noopener noreferrer"/>}}>{m.content}</ReactMarkdown></div></div>)}
