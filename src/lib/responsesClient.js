...
const DEFAULT_MODEL = import.meta.env.VITE_OPENAI_MODEL || "gpt-5.4-mini";
...
async function createResponse(input, apiKey) {
...
export async function runProfileChat(messages, { apiKey } = {}) {
  const resolvedApiKey = apiKey || DEFAULT_API_KEY;

  if (!resolvedApiKey) {
    throw new Error(
      "VITE_OPENAI_API_KEY is not configured for browser chat. Add it to your deployment environment."
    );
  }
  ...
  if (normalizedMessages.length === 0) {
    throw new Error("A conversation payload is required.");
  }
  ...
  for (let iteration = 0; iteration < 6; iteration += 1) {
    response = await createResponse(input, resolvedApiKey);
