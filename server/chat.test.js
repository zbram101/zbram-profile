...
test('passes real conversation history and server-side instructions',async()=>{const history=[...question,{role:'assistant',content:'Yes, at Kaiser.'},{role:'user',content:'What did he do there?'}];let calls=0;const r=await handleChat(request(history),{OPENAI_API_KEY:'private-test-value'},async(url,options)=>{calls++;assert.equal(url,'https://api.openai.com/v1/responses');const body=JSON.parse(options.body);assert.deepEqual(body.input,history);assert.equal(body.store,false);assert.match(body.instructions,/Globality/);assert.equal(options.headers.Authorization,'Bearer private-test-value');return Response.json({output:[{type:'message',content:[{type:'output_text',text:'He built web applications.'}]}]});});assert.equal(calls,1);assert.deepEqual(await r.json(),{text:'He built web applications.',mode:'ai'});});
